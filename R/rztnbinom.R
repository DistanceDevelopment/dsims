...
rztbinom <- function(n.vals, mean = NA, var = NA){
  # error functions like this
  # if(is.na(mean) | (mean <= 1)) {
  #   warning("NAs produced")
  #   return(rep(NaN, n))
  # } else {

  # calculates the mean of the ztnbinom function based on the 
  # input parameters, N and p
  ...
  # value, k, in the zero-truncated negative binomial function with the 
  # specified N and p
  ...
  for (i in 1:upper) {
    return_vals[i] <- i
    cdf_vals[i] <- cdf_ztbinom_book(k = i, N = est_params$est_N, p = est_params$est_p)
  }
  ...
  min = 0
  max = cdf_ztbinom_book(k = upper, N = est_params$est_N, p = est_params$est_p) # can do e^huge
  # max = 1
