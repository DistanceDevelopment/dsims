store.dht.results <- function(results, dht.results, i, clusters, data, obs.tab, sample.tab){
  ...
  if(clusters){
    for(strat in seq(along = strata.names)){
      ...
      #results$expected.size[strat, c("Expected.S","se.Expected.S"), i] <- as.matrix(dht.results$Expected.S[dht.results$Expected.S$Region == strata.names.ND[strat], c("Expected.S","se.Expected.S")])
      # To be backwards compatible with mrds which for single strata outputs 2 rows and CRAN R-devel doesn't
      # currently allow the rows to be subset!
      tmp <- as.matrix(dht.results$Expected.S[dht.results$Expected.S$Region == strata.names.ND[strat], c("Expected.S","se.Expected.S")])
      exp.S <- tmp[1,1]
      se.exp.S <- tmp[1,2]
      results$expected.size[strat, c("Expected.S","se.Expected.S"), i] <- c(exp.S, se.exp.S)
    }
  }
  return(results)
}
